...
public class Inventario extends JFrame implements ActionListener{
	...
	public void aumentarStock(Vacuna x, int cant) {
		...
		if (busqueda != -1) {
			for(int i = 0 ;aux != null ; aux =aux.getNext(),i++) {
				if (aux.getData().equals(x)) {
					aux.getData().aumentarStockVacuna(cant);
				}
			}	
		}
		else {
	...
	public int cantidadVacunas(){
		int cant=0;
		for(int i=0;i<inventarioVacuna.length();i++) {
			cant += this.inventarioVacuna.searchIndex(i).getCantidad();
		}		
		return cant;
	}
